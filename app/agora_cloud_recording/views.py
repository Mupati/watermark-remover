...
from . import agora_cloud_recording
from ..models import User
from .agora_key.RtcTokenBuilder import RtcTokenBuilder, Role_Attendee
from .agora_key.RtmTokenBuilder import RtmTokenBuilder, Role_Rtm_User


@agora_cloud_recording.route('/agora-cloud-recording')
@login_required
def index():
    users = User.query.all()
    all_users = [user.to_json() for user in users]
    return render_template('agora_cloud_recording/index.html', title='Agora Video Call with RTM', allUsers=all_users, agoraAppID=os.environ.get('AGORA_APP_ID'))


@agora_cloud_recording.route('/users')
def fetch_users():
    users = User.query.all()
    all_users = [user.to_json() for user in users]
...
@agora_cloud_recording.route('/agora-cloud-recording/token',  methods=['POST'])
...
@agora_cloud_recording.route('/agora-cloud-recording/resource-id', methods=['POST'])
...
@agora_cloud_recording.route('/agora-cloud-recording/start-recording', methods=['POST'])
def start_recording():
    auth_user = current_user.to_json()
    userId = auth_user['id']
    channelName = request.json['channelName']
...
@agora_cloud_recording.route('/agora-cloud-recording/stop-recording', methods=['POST'])
def stop_recording():
    auth_user = current_user.to_json()
    userId = auth_user['id']
    channelName = request.json['channelName']
