...
def convert_images_to_pdf(pil_images: list, filename):
    processed_file_path = os.path.join(
        app.config['UPLOAD_FOLDER'], 'processed_'+filename+'.pdf')
...
def proccess_file():
    ...
    if uploadedFile and allowed_file(uploadedFile.filename):
        ...
        stored_file = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        ...
        return jsonify(status="success", message="Conversion complete",
                       processed_file_path=url_for('processed_file_dir', filename=processed_file)), 200
    else:
